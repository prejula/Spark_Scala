...
object SparkSqlHive {
  ...
  {
    val sparkConf = new SparkConf().setAppName("SparkSqlHive");
    ...
    val createTable = hiveSqlContext.sql("create table ATab(col1 INT, col2 String, col3 INT) row format delimited fields terminated by \" \" lines terminated by \"\n\" ");
    val loadData = hiveSqlContext.sql("load data local inpath \"/home/ubuntu/spark_examples/a.txt\" into table ATab");
